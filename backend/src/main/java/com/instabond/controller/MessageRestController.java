...
public class MessageRestController {
        ...
        @Operation(summary = "Get conversation history", description = "Returns paginated messages in ascending time order for the provided conversation.")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Conversation history returned successfully", content = @Content(schema = @Schema(implementation = ChatMessageResponse.class))),
        ...
        public ResponseEntity<List<ChatMessageResponse>> getHistory(
                        @Parameter(description = "Conversation ID", example = "64f1a2b3c4d5e6f7a8b9c0d2") @PathVariable String conversationId,
                        @Parameter(description = "Zero-based page index", example = "0") @RequestParam(defaultValue = "0") int page,
                        @Parameter(description = "Page size (service enforces a safe upper bound)", example = "30") @RequestParam(defaultValue = "30") int size,
        ...
        @Operation(summary = "Upload chat image", description = "Uploads an image file, stores it as a chat message, then broadcasts it to the conversation topic over WebSocket.")
        @ApiResponses({
        ...
        public ResponseEntity<ChatMessageResponse> uploadChatImage(
                        @Parameter(description = "Conversation ID", example = "64f1a2b3c4d5e6f7a8b9c0d2") @PathVariable String conversationId,
                        @Parameter(description = "Image file to send", required = true) @RequestPart("file") MultipartFile file,
                        @AuthenticationPrincipal UserDetails userDetails) {
                ...
                // Call Notification Service to send push notification to offline users ("User X
                // sent a photo")

                return ResponseEntity.status(201).body(response);
        }

        @Operation(summary = "Mark conversation messages as read", description = "Adds read receipts for unread messages in the conversation for the current user.")
        @ApiResponses({
